...
class EchoClient(protocol.Protocol):
    ...
    def connectionMade(self):
        ...
        argument.token = 'token123456'
        self.dateSend(argument, 4)

    def dataReceived(self, data):
        """As soon as any data is received, write it back."""
        command, message = resolveRecvdata(data)
        response = CharacterLoginResponse()
        response.ParseFromString(message)
        if not response.result and response.message == 'no_role':
            request = CreateCharacterRequest()
            nickname = raw_input("请输入你的昵称：")
            request.nickname = nickname
            self.dateSend(request, 5)

        elif response.result:
            print response.character_info
