...
function CCBReaderLoad(strFilePath,proxy,owner)
    ...
    ccbReader:setCCBRootPath(CONFIG_RES_PAHT)
    local node      = ccbReader:load(CONFIG_RES_PAHT .. strFilePath)
    local rootName  = ""
    ...
    for i = 1 , table.getn(nodesWithAnimationManagers) do
        ...
        if not owner[documentControllerName] then owner[documentControllerName] = {} end

        owner[documentControllerName]["mAnimationManager"] = animationManager
        ...
        for i = 1,table.getn(documentCallbackNames) do
            ...
            if "" ~= documentControllerName then
                if "function" == type(owner[documentControllerName][callbackName]) then
                    proxy:setCallback(callbackNode, owner[documentControllerName][callbackName], documentCallbackControlEvents[i])
                else
        ...
        for i = 1, table.getn(documentOutletNames) do
            ...
            owner[documentControllerName][outletName] = tolua.cast(outletNode, proxy:getNodeTypeName(outletNode))
        end
        --[[
        ...
        for i = 1 , table.getn(keyframeCallbacks) do
            ...
            if 1 == callbackType and nil ~= owner[documentControllerName] then
                local callfunc = cc.CallFunc:create(owner[documentControllerName][callbackName])
