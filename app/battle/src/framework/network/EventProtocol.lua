...
function EventProtocol.extend(object)
    ...
    --print("module api.EventProtocol is deprecated, please use cc.components.behavior.EventProtocol")

    object.listeners_ = {}
    object.listenerHandleIndex_ = 0

    function object:addEventListener(eventName, listener, target)
        cclog("__________object:addEventListener")
        ...
        if ttarget == "table" or ttarget == "userdata" then
            cclog("api.EventProtocol.addEventListener(eventName, listener, target) is deprecated, please use api.EventProtocol.addEventListener(eventName, handler(target, listener))")
