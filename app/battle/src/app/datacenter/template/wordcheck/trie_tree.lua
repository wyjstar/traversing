...
checkWord = checkWord or {}

function checkWord.add_ch(ch, node)
...
function checkWord.is_end(n, node)
...
function checkWord.get_child(ch, node)
    return node._child[ch]
end

function checkWord.replace_bad_word(text, mark)
    ...
    for k=0, #li do
        ...
        node = root
        index = k
        node = checkWord.get_child(ch, node)

        while node do
            n = li[k+1]
            if checkWord.is_end(n, node) then
                for m=k, index do
                    li[m] = mark
                end
                break
            ...
            index = index + 1
            node = checkWord.get_child(li[index], node)
        end
    end
...
function checkWord.get_bad_word(text)
