...
local PVOtherPlayerEquipAttribute = class("PVOtherPlayerEquipAttribute", BaseUIView)
g_seat = nil
g_equipData = nil

function PVOtherPlayerEquipAttribute:ctor(id)
    PVOtherPlayerEquipAttribute.super.ctor(self, id)

    -- self.lineupData = getDataManager():getLineupData()
    self.c_Calculation = getCalculationManager():getCalculation()
end

function PVOtherPlayerEquipAttribute:onMVCEnter()
    ...
    -- self.shopEquipTips = getDataManager():getShopData():getShopEquipTips()
    -- getDataManager():getShopData():setShopEquipTips(nil)
    -- if self.shopEquipTips ~= nil and table.getn(self.shopEquipTips) > 1 then
    --     self.tipsNum = table.getn(self.shopEquipTips)
    --     for k,v in pairs(self.shopEquipTips) do 
    --         if v == self.equipData then self.curPage = k end
    --     end
    --     if self.curPage == 1 then 
    --         self.menuRight:setVisible(true)
    --     elseif self.curPage == self.tipsNum then
...
-- 获取控件与设置属性
function PVOtherPlayerEquipAttribute:initView()
    ...
    self.labelEquipTo:setVisible(false)
    self.labelEquipToTip:setVisible(false)
    ...
    -- if self.type == 101 then    -- 从掉落获得查看过来
    --     self.getButton:setVisible(false)
    --     self.menuChange:setVisible(false)
    --     self.menuDown:setVisible(false)
    --     self.equipMenuA:setVisible(false)
    ...
    self.getButton:setPosition(cc.p(_posx+200,_posy))
    ...
    -- local _reshd = equipTemp:getEquipResHD(self.equipData.no)
    -- local _icon = equipTemp:getEquipResIcon(self.equipData.no)
    ...
    -- self.labelEquipLv:setString(self._level)
...
function PVOtherPlayerEquipAttribute:initData()
    ...
    local _ratio = getTemplateManager():getBaseTemplate():getReturnCoinRatio()*100
    local _tips = string.format(Localize.query("equip.10"), _ratio)
    self.labelTips:setString(_tips)
    ...
    -- self.labelEquipLv:setString(self._level)
    local levelNode = getLevelNode(self._level)
    ...
    -- if _equipedWho == Localize.query("equip.1") then
    --     self.labelEquipToTip:setString("未装备")
    --     self.labelEquipTo:setString("")
    --     self.labelEquipToTip:setColor(ui.COLOR_MISE)
    -- else
...
function PVOtherPlayerEquipAttribute:setChageModel(flag)
...
function PVOtherPlayerEquipAttribute:initTouchListener()

    local function backMenuClick()
        getAudioManager():playEffectButton2()
        self:onHideView()

        -- stepCallBack(G_GUIDE_50102)               -- 50008 点击返回
        -- stepCallBack(G_GUIDE_40129)
    ...
    local function strengthenMenuClick()
        ...
        ----------------  传承 ---------------
        local _inherit = self.funcTable[4]
        print("strengthen menu click ", _inherit)
        if _inherit ~= nil then
            if _inherit == 0 then
                self:onHideView(-1)
        ...
        self:onHideView(-1)
        getModule(MODULE_NAME_HOMEPAGE):showUIView("PVEquipmentStrengthen", g_equipData)
    ...
    function onChangeClick()
        ...
        --------------------------------------
        g_equipData = self.funcTable[1]
        getAudioManager():playEffectButton2()
        self:onHideView(-1)
        getModule(MODULE_NAME_LINEUP):showUIView("PVSelectEquipment", g_seat, -1, g_equipData)
        g_seat = nil
        g_equipData = nil
    end
    ...
    function onGetSoldierClick()
        local equToGetId = getTemplateManager():getEquipTemplate():getTemplateById(self.equipData.no).toGet
        local _data =  self.c_ChipTemplate:getDropListById(equToGetId)
        ...
        -- if _data and type(_data) == "table" and table.nums(_data) == 0 then
            ...
            getOtherModule():showOtherView("PVChipGetDetail", _data, self.equipData.no, 3)
        end
     end
    ...
    self.ccbiNode["UIEquipAttribute"]["onLeftMove"] = onLeftMove
    self.ccbiNode["UIEquipAttribute"]["onRightMove"] = onRightMove
end

function PVOtherPlayerEquipAttribute:createScrollView()
...
function PVOtherPlayerEquipAttribute:createBasicAttribute()

    local languageTemp = getTemplateManager():getLanguageTemplate()
    local equipTemp = getTemplateManager():getEquipTemplate()
    local equipTempItem = equipTemp:getTemplateById(self.equipData.no)
    local squipTemp = getTemplateManager():getSquipmentTemplate()
    ...
    local _descrip = ""
    local isJB = false
    local main_attr = self.c_Calculation:EquMainAttr(self.equipData)
    ...
    if mainNum == 0 then
        ...
        isJB = true
        _descrip = _descrip .. main_attr.AttrName.. " +" .. main_attr.Value .. main_attr.Scope
    end
    local theHight = 0
    if isJB then
        ...
        local node1 = CCBReaderLoad("equip/ui_equip_attri_item1.ccbi", proxy, self.attrNode)
    ...
    local minor_attr = self.c_Calculation:EquMinorAttr(self.equipData)
    ...
    if ciNum == 0 then
        ...
        isCB = true
        for i=1,ciNum do
            _descrip = _descrip .. minor_attr[i].AttrName.. " +" .. minor_attr[i].Value .. minor_attr[i].Scope
            -- if i%2 == 0 then
                _descrip = _descrip.."\n"
            -- end
        end
    ...
    if isCB then
        ...
        self.attrNode = {}
        local proxy = cc.CCBProxy:create()
        local node2 = CCBReaderLoad("equip/ui_equip_attri_item2.ccbi", proxy, self.attrNode)
    ...
    -- 羁绊
    local links = equipTemp:getLinks(self.equipData.no)
...
function PVOtherPlayerEquipAttribute:initHeroLayerTouch()

    self.fbLayer = self.ccbiRootNode["fb_layer"]
    self.fbLayer:setTouchMode(cc.TOUCHES_ONE_BY_ONE)
    self.fbLayer:setTouchEnabled(true)
    local posX,posY = self.fbLayer:getPosition()
    local size = self.fbLayer:getContentSize()
    ...
    local function onTouchEvent(eventType, x, y)
        if eventType == "began" then
            local _point = self.fbLayer:convertToNodeSpace(cc.p(x,y))
    ...
    self.fbLayer:registerScriptTouchHandler(onTouchEvent)
    self.fbLayer:setTouchMode(cc.TOUCHES_ONE_BY_ONE)
    -- self.heroLayer:setTouchEnabled(false)
end

function PVOtherPlayerEquipAttribute:onLayerTouchCallBack(moveType)
...
--往左滑动
function PVOtherPlayerEquipAttribute:OnTouchMoveLeft()
    if self.curPage == 1 then
        self.equipData = self.shopEquipTips[1]
        ...
        self.equipData = self.shopEquipTips[self.curPage]
    end
    self.svContaner:removeAllChildren()
    cclog("----往左滑动＝＝＝＝＝"..self.curPage)
    self:initData()
    self:createBasicAttribute()
end

--往右滑动
function PVOtherPlayerEquipAttribute:OnTouchMoveRight()

    if self.curPage == self.tipsNum then
        self.equipData = self.shopEquipTips[self.tipsNum]
    ...
    self.svContaner:removeAllChildren()
    cclog("----往右滑动＝＝＝＝＝"..self.curPage)
    self:initData()
    self:createBasicAttribute()
end
return PVOtherPlayerEquipAttribute
