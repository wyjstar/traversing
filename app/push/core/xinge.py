...
class TimeInterval(object):
    ...
    def GetObject(self):
        ...
        if not self._isValidInterval():
            return None
        return {self.STR_START: {self.STR_HOUR: str(self.startHour),
                                 self.STR_MIN: str(self.startMin)},
                self.STR_END: {self.STR_HOUR: str(self.endHour),
                               self.STR_MIN: str(self.endMin)}}
...
class ClickAction(object):
    ...
    def __init__(self, actionType=1, url='', confirmOnUrl=0,
                 activity='', intent=''):
...
class Style(object):
    ...
    def __init__(self, builderId=0, ring=0, vibrate=0,
                 clearable=1, nId=N_INDEPENDENT):
...
class XingeApp(object):
    ...
    def SetPushParams(self, params, message, environment):
        ...
        if None == msgObj or not isinstance(msgObj, dict):
            ...
            params['message'] = json.dumps(msgObj, separators=(',', ':'),
                                           ensure_ascii=False)
            return True
    ...
    def QueryPushStatus(self, pushIdList):
        ...
        if ERR_OK == ret[0]:
            ...
            for status in ret[2]['list']:
                result[status['push_id']] = MessageStatus(status['status'],
                                                          status['start_time'])
...
class XingeHelper(object):
    ...
    def GenSign(cls, path, params, secretKey):
        ...
        signSource = '%s%s%s%s%s' % (cls.HTTP_METHOD, cls.XINGE_HOST,
                                     path, paramStr, secretKey)
    ...
    def Request(cls, path, params):
        httpClient = httplib.HTTPConnection(cls.XINGE_HOST,
                                            cls.XINGE_PORT,
                                            timeout=cls.TIMEOUT)
        if cls.HTTP_METHOD == 'GET':
            httpClient.request(cls.HTTP_METHOD,
                               ('%s?%s' % (path, urllib.urlencode(params))),
                               headers=cls.HTTP_HEADERS)
        elif cls.HTTP_METHOD == 'POST':
            httpClient.request(cls.HTTP_METHOD, path, urllib.urlencode(params),
                               headers=cls.HTTP_HEADERS)
        else:
            # invalid method
...
def PushAccountAndroid(accessId, secretKey, title, content, account):
    x = XingeApp(accessId, secretKey)
    return x.PushSingleAccount(0, account,
                               _BuildAndroidNotification(title, content))
