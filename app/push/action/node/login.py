...
@nodeservice_handle
def register_push_message(dynamic_id, device_token):
    """
    消息推送注册
...
@nodeservice_handle
def set_push_switch(dynamic_id, msg_type, oper):
    """
    设置推送消息开关
    @param msg_type: 消息类型
    @param oper: 0关闭，1打开
...
@nodeservice_handle
def add_push_message(dynamic_id, msg_type, message, send_time):
    """
    添加发送推送消息的任务
    @param msg_type: 消息类型
    @param message: 发送消息
...
@nodeservice_handle
def online_offline(dynamic_id, on_or_off):
    """
    在线离线通知
