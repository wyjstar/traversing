...
def become_friends(dynamic_id, target_id, **kwargs):
    ...
    player1 = tb_character_friend.getObjData(player.base_info.id)
    player2 = tb_character_friend.getObjData(target_id)
    if not player1:
        data = {'id': player.base_info.id, 'friends': [], 'blacklist': [], 'applicants_list': {}}
        tb_character_friend.new(data)
    if not player2:
        data = {'id': target_id, 'friends': [], 'blacklist': [], 'applicants_list': {}}
        tb_character_friend.new(data)
    ...
    player_friend_data = tb_character_friend.getObjData(target_id)
    player_friends = player_friend_data.get('friends')
