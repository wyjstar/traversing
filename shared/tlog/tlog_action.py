...
def player_register(player_data, handler):
    log4tx.player_register(GameSvrId=game_server_id, dtEventTime=xtime.strdatetime(), GameAppID=game_app_id, PlatID=handler.plat_id, Uid=player_data.id, ClientVersion=handler.client_version,
...
def player_login(player_data, handler):
    log4tx.player_login(Uid=player_data.id, GameSvrId=game_server_id, dtEventTime=xtime.strdatetime(), GameAppID=game_app_id,
                        PlatID=handler.plat_id, Level=player_data.level, PlayerFriendsNum=1,
...
def item_flow(player_data, addorreduce, itemtype, itemnum, itemid, itid, reason, after_num):
    log4tx.item_flow(GameSvrId=game_server_id, dtEventTime=xtime.strdatetime(), \
                                     GameAppID=game_app_id, Uid=player_data.base_info.id, ItemID=itemid, ItemType=itemtype, \
